...
CONFIG_DIR = os.path.join(os.environ["LOCALAPPDATA"], "temp", "ExternalCheat")
...
class ConfigWindow(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        main_layout.addWidget(trigger_container)
        main_layout.addWidget(aim_container)
    ...
    def create_trigger_container(self):
        ...
        self.trigger_bot_active_cb = QtWidgets.QCheckBox("Enable")
        self.trigger_bot_active_cb.setChecked(
            self.settings["trigger_bot_active"] == 1
        )
        ...
        trigger_layout.addWidget(QtWidgets.QLabel("Key:"))
    ...
    def create_esp_container(self):
        ...
        self.hp_bar_rendering_cb.setChecked(
            self.settings["hp_bar_rendering"] == 1
        )
        self.hp_bar_rendering_cb.stateChanged.connect(self.save_settings)
        esp_layout.addWidget(self.hp_bar_rendering_cb)
 
        self.head_hitbox_rendering_cb = QtWidgets.QCheckBox("Hitbox")
        self.head_hitbox_rendering_cb.setChecked(
            self.settings["head_hitbox_rendering"] == 1
        )
...
def configurator():
    ...
    nord_style = """
        ...
        /* Checkbox Style */
        QCheckBox {
            color: #88C0D0;  /* Light blue for checkboxes */
            font-size: 14px;
            padding-left: 5px;  /* Reduced padding to shift checkbox left */
        }
 
        QCheckBox::indicator {
            width: 16px;
            height: 16px;
            margin-left: -5px;  /* Adjusts the left margin of the indicator */
        }
 
        QCheckBox::indicator:unchecked {
            border: 2px solid #88C0D0;  /* Light blue border */
            background: #3B4252;  /* Darker background */
        }
 
        QCheckBox::indicator:checked {
            background-color: #88C0D0;  /* Light blue background when checked */
            border: 2px solid #88C0D0;  /* Matching border */
        }
 
        /* Line Edit / Input Field Style */
        QLineEdit {
            background-color: #3B4252;  /* Dark input background */
        ...
        /* Button Style */
        QPushButton {
            background-color: #5E81AC;  /* Light blue background */
        ...
        QPushButton:hover {
            background-color: #81A1C1;  /* Slightly lighter blue */
        }
        ...
        QSlider::handle:horizontal {
            background-color: #88C0D0;  /* Light blue handle */
            border-radius: 5px;
            width: 16px;
            height: 16px;
        }
        ...
        QSlider::add-page:horizontal {
            background-color: #4C566A;  /* Light gray track */
        }
 
        QSlider::sub-page:horizontal {
            background-color: #81A1C1;  /* Blue track */
        }
    ...
    def update_scene(self):
        ...
        try:
            ...
            fps_text = self.scene.addText(
                f"Optical Softwares | FPS: {self.fps}",
                QtGui.QFont("Noto Sans", 12, QtGui.QFont.Bold),
            )
            fps_text.setPos(5, 5)
            fps_text.setDefaultTextColor(QtGui.QColor(94, 129, 172))
        ...
        try:
            ...
            try:
                ...
                if settings["line_rendering"] == 1:
                    ...
                    bottom_left_x = (
                        head_pos[0] - (head_pos[0] - leg_pos[0]) // 2
                    )
                    bottom_y = leg_pos[1]
                    line = scene.addLine(
                        bottom_left_x,
                ...
                if settings["hp_bar_rendering"] == 1:
                    ...
                    max_armor_hp = 100
                    armor_hp_percentage = min(
                        1.0, max(0.0, armor_hp / max_armor_hp)
                    )
                    armor_bar_width = 2
                    ...
                    current_armor_height = (
                        armor_bar_height * armor_hp_percentage
                    )
                    armor_bar_y_bottom = (
                        armor_bar_y_top
                        + armor_bar_height
...
def draw_bones(scene, pm, bone_matrix, view_matrix, width, height):
    ...
    try:
        ...
        for connection in bone_connections:
            if (
                connection[0] in bone_positions
                and connection[1] in bone_positions
            ):
                scene.addLine(
...
if __name__ == "__main__":
    print("Waiting for Counter-Strike 2")
    while True:
        time.sleep(1)
        try:
            pm = pymem.Pymem("cs2.exe")
